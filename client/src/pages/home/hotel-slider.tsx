...
export default function HotelSlider() {
    ...
    const nextSlide = async () => {
        ...
        let increment = 0
        for(let i = slideIndex; i < limit && i < hotels.length; i ++) {
            slide.push(hotels[i])
            increment++;
        }

        if(slideIndex + increment >= hotels.length) setSlideIndex(0);
    ...
    useEffect(() => {
        (async () => {
            const response = await fetch(APP_SETTINGS.backend + "/api/get/hotel", {
                method: 'GET',
            });
            const hotelData = await response.json();
            var hotelsData = [...hotelData];
            
            for (let i = 0; i < hotelData.length; i++) {
                const response2 = await fetch(APP_SETTINGS.backend + "/api/get/room/from/hotel", {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({'id': hotelsData[i].ID}),
                });
                const roomData = await response2.json();
                hotelsData[i].rooms = roomData
            }
    ...
    return (
        ...
        <h1 className="font-main font-serif fc-a fs-3xl font-bold m-0">Hotels</h1>
        ...
        <div className="w-100 h-60 flex-center mt-10">
            ...
            <div className="w-80 h-s50 mobile-flex-col gap-m scroll-simple ml-20">
                {
                    slides.map((hotel : any, index : number) => {
                        return (
                            <HotelCard 
                                hotel={hotel}
                                key={index} className={`w-25 h-100 flex-shrink-0 shadow`} style={slideStyle[index]}
                            />
                        )
                    })
                }
