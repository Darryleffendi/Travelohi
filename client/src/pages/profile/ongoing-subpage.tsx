...
export default function OngoingSubpage({setLeftCard} : IProfileSubpage) {
    ...
    const fetchSeats = async () => {
        const response = await fetch(APP_SETTINGS.backend + "/api/get/seat/ongoing", {
            method: 'POST',
    ...
    const fetchRooms = async () => {
        const response2 = await fetch(APP_SETTINGS.backend + "/api/get/room/ongoing", {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            credentials: 'include',
            body: JSON.stringify({
    ...
    return (
        <>
            <p className="fs-l m-0 font-serif font-medium fc-a">Your Ongoing Orders</p>
            ...
            <div className={`transition-2 ${showHotels ? "h-auto" : "h-0 overflow-hidden"}`}>
            {
                query == "" ? (
                    hotelCart.map((hotel) => {
                        return <HotelCartCard hotelCart={hotel} selectRoom={() => {}} refreshRoom={fetchRooms} disableUpdate={true}/>
                    })
                ) : (
