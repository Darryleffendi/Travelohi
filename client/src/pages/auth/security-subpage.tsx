...
export default function SecuritySubpage({setSubpage, addNewData, newData, submitForm, errorMessage} : IAuthPageParameters) {
    ...
    return (
        <div className="w-80 flex-center flex-column justify-between wrap h-80 transition" style={{opacity:`${opacity}`}}>

            <div className="flex-center flex-column justify-between wrap">
                ...
                <p className="font-p fc-gray m-0 mb-2 fs-xxs">Please enter your security details</p>                
            </div>

            <div className="w-100">
                <label className="font-p fc-gray fs-xxs text-left w-100 mt-2" htmlFor="password">Password</label>
                <input 
            ...
            <div className="w-100">
                <label className="font-p fc-gray fs-xxs text-left w-100 mt-2" htmlFor="confirm">Confirm Password</label>
                <input 
            ...
            <div className="w-100">
                <label className="font-p fc-gray fs-xxs text-left w-100" htmlFor="question">Security Question</label>
                <select 
            ...
            <div className="w-100">
                <label className="font-p fc-gray fs-xxs text-left w-100" htmlFor="answer">Security Answer</label>
                <input 
            ...
            <div className="flex-center w-100 mt-2">
                ...
                />
                <label className="font-p fs-xxs fc-gray text-left w-80 ml-1" htmlFor="agree">I want to receive newsletters and emails</label>
            </div>
            
            <p className="font-p fs-xxs fc-red mb-2" id="error_login">{errorMessage}</p>
            
            <button className={`${styles.signBtn} fs-xs font-h bg-col-a pointer mb-2`} onClick={() => { if(submitForm) submitForm()}}>Register</button>

            <div>
                <p className="font-p fc-gray fs-xxs inline">Already have an account? </p>
                <a className="font-h fc-black fs-xxs underline pointer" onClick={() => changePage(0)}>Sign in</a>
            </div>
        </div>
    )
}