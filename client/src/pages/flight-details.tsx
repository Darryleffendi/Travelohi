...
export default function FlightDetails() {
    ...
    for(let i = 0; i < selectedSeats.length; i++) {
        ...
        seats += " " + alphabet[selectedSeats[i].columnIndex] + (selectedSeats[i].rowIndex)
    }
    ...
    useEffect(() => {
        ...
        flight.seatingChart.forEach((row : Array<any>, i : number) => {
            ...
            row.forEach((seat, j) => {
                ...
                if(row.length == 6 && j == 3) {
                    rowElement.push (<div className="w-40p flex-center justify-center">{i + 1}</div>)
                }
    
                if(row.length == 10 && (j == 3 || j == 7)) {
                    rowElement.push (<div className="w-40p flex-center justify-center fc-a o-30 font-medium">{i + 1}</div>)
                }
                ...
                rowElement.push(
                    <div 
                        className={`w-40p h-40p flex-center justify-center mb-1 rounded ${seatClass}`}
                        onClick={() => onClick()}
                    >
            ...
            <div className="w-screen bg-col-main flex justify-center">
                <div className="w-40">
                    ...
                    >
                        ...
                        <div className="w-90 absolute">
                            <div className="w-100 mt-2 flex-center justify-start">
                                ...
                                <div className="w-20p flex-shrink-0 flex justify-center">
                                    <div className="w-1p h-s60 border-left border-a" style={{borderStyle: "dashed", borderWidth: "0px 2px 0px 0px"}}></div>
                                </div>
                        ...
                        <div className="w-90 mt-2 z-1">
                            <div className="w-100 h-78p flex-center justify-start">
                                ...
                                <div className="w-70 ml-2 flex-col flex-start bg-col-white">
                                    ...
                                    <div className="mt-1">
                                        <input 
                                            type="text" className={`${styles.overlayInput} fs-2xs`} id="email" 
                                            style={{backgroundImage: `url(${baggageIcon}`}} placeholder="Input Baggage (in kg)"
                                            value={baggage} onChange={(e) => setBaggage(e.target.value)}
                                        />
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="absolute w-90 h-64p bottom-0 bg-col-white z-5 flex-center justify-between mb-1">
                            <div>
                                <p className="fs-xl fc-a2 font-bold m-0">${price}</p>
                                <p className="mt--1 m-0 fs-3xs o-30 h-op3">Baggage costs $1/kg</p>
                            </div>
                            <div className="w-30 h-70 bg-col-a2 flex-center justify-center font-bold fc-white rounded o-70 h-op2 pointer">
                                Check Out
                            </div>
                        </div>
                    </div>
                ...
                <div className={`${styles.chartMobile} mobile`}>
                    ...
                    <div className="w-100 h-s140"></div>
                    <div className="w-100 h-s140"></div>
